...
export const Navigation = () => {
  ...
  const handleMouseMove = (event: MouseEvent) => {
    ...
    let newWdith = event.clientX;

    if (newWdith < 240) newWdith = 240;
    if (newWdith > 480) newWdith = 480;

    if (sidebarRef.current && navbarRef.current) {
      sidebarRef.current.style.width = `${newWdith}px`;
      navbarRef.current.style.setProperty("left", `${newWdith}px`);
      navbarRef.current.style.setProperty(
        "width",
        `calc(100% -${newWdith}px )`
      );
    }
  };
  ...
  return (
    <>
      <aside
        ref={sidebarRef}
        className={cn(
          "group/sidebar cursor-pointer h-full bg-secondary overflow-y-auto relative flex w-60 flex-col z-[99999]",
      ...
      <div
        ...
        className={cn(
          ...
          isResetting && "tanstion-all ease-in-out duration-300",
